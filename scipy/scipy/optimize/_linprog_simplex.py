...
def _pivot_col(T, tol=1e-9, bland=False):
    ...
    """
    print("Function to optimize:", T[-1])

    print("Using original pivoting, minimum coefficient pivoting")
    ...
    # print("Using max coefficient pivoting")
    # ma = np.ma.masked_where(T[-1, :-1] >= -tol, T[-1, :-1], copy=False)
    # if ma.count() == 0:
    #     return False, np.nan
    # if bland:
    #     return True, np.nonzero(np.logical_not(np.atleast_1d(ma.mask)))[0][0]
    # return True, np.ma.nonzero(ma == ma.max())[0][0]
