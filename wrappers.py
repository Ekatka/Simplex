...
class SwitchBonusWrapper(gym.Wrapper):
    """Reward the agent by `bonus` whenever its action differs from the previous step.

    Encourages strategy diversity — useful when combined with BaselineRewardWrapper
    to push the agent past a "imitate the baseline" plateau by forcing it to
    sometimes try a different rule. `FullPivotEnv` already has this built in;
    this wrapper provides the same behavior for the phase-2-only
    `RandomMatrixEnv` path.
    """

    def __init__(self, env, bonus: float = 0.3):
        super().__init__(env)
    ...
    def step(self, action):
        ...
        if self.bonus > 0 and self._last_action is not None and a != self._last_action:
            reward += self.bonus
        self._last_action = a
