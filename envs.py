...
class CubeEnv(SecondPhasePivotingEnv):
    """Klee-Minty / Jeroslow / Goldfarb-Sit cube training environment.

    On each reset, samples one of the three cube types uniformly at random
    and builds the corresponding Phase-2 tableau (origin BFS with slacks
    basic). The tableau shape is fixed at (n+1, 2n+1) across all cubes, so
    the Dict observation space is constant across resets and across parallel
    envs in a vec_env.

    The agent uses the standard `PIVOT_MAP` action space and solves each
    cube as a regular phase-2 simplex problem; `cube_type` and `cube_param`
    are surfaced in the step-info dict so evaluation code can slice results
    per cube family.
    """

    def __init__(self, n, seed=None, eps_range=None, beta_range=None,
                 delta_range=None):
        from cubes import sample_cube
        from config import CUBE_EPS_RANGE, CUBE_BETA_RANGE, CUBE_DELTA_RANGE
        self.n = int(n)
        self._rng = np.random.default_rng(seed)
        self._eps_range = eps_range if eps_range is not None else CUBE_EPS_RANGE
        self._beta_range = beta_range if beta_range is not None else CUBE_BETA_RANGE
        self._delta_range = delta_range if delta_range is not None else CUBE_DELTA_RANGE
        T, basis, ct, p = sample_cube(
            self.n, self._rng,
            eps_range=self._eps_range,
            beta_range=self._beta_range,
            delta_range=self._delta_range,
        )
        self.cube_type = ct
        self.cube_param = p
        super().__init__(T, basis)
    ...
    def reset(self, seed=None, **kwargs):
        ...
        self.cube_type = ct
        self.cube_param = p
        self.remove_artificial()  # no-op: slacks-only basis
        return super().reset(seed=seed)

    def step(self, action):
        obs, reward, done, truncated, info = super().step(action)
        info["cube_type"] = self.cube_type
        info["cube_param"] = self.cube_param
        return obs, reward, done, truncated, info
...
class FullPivotEnv(gym.Env):
    ...
    def step(self, action):
        ...
        tol = self.tol
        reward = -self._step_cost(strategy)
        # Switch bonus: reward for picking a different strategy than the previous step
        if self.switch_bonus > 0 and self._action_hist and a != self._action_hist[-1]:
            reward += self.switch_bonus
...
class LeducFullPivotEnv(FullPivotEnv):
    ...
    def __init__(self, game_name: str, alpha: float = 2.0, num_ranks: int = 3,
                 ...
                 use_baseline: bool = True, baseline_coef: float = 5.0,
                 maxiter: int = 50_000, tol: float = 1e-7,
                 switch_bonus: float = 0.0, seed=None):
        import pyspiel
        from leduc_experiment import build_sequence_form_matrices, sample_rank_weights
        self._game = pyspiel.load_game(game_name)
